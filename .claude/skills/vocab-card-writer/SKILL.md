...
單字卡放在「年份/來源」子資料夾。**來源**與**主題檔名**是兩個獨立的概念，不一定相同：
- **來源**：學習素材的名稱，作為資料夾名稱（如 `2026/AI Fluency`、`2026/Adv 2603`）
- **主題檔名**：主題檔案的名稱，可以比來源更具描述性（如 `Introduction to AI Fluency`）

確認規則：
- 使用者明確指定來源 → 使用對應名稱
- 使用者提供主題名稱但未指定來源 → 詢問：「來源資料夾要用『{主題名稱}』，還是放在 misc？」
- 使用者完全未指定 → 使用 `{年份}/misc`
- 同一對話中已確認過 → 沿用，不重複詢問
