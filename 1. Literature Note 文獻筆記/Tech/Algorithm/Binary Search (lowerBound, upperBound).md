...
| 單調性模式           | 找什麼    | 稱呼     | 搜尋方向 | mid           | 否定式確認                      |
| --------------- | ------ | ------ | ---- | ------------- | -------------------------- |
| `[F F F T T T]` | 第一個 T  | **左界** | 向左   | `mid = floor` | 是否限縮在右邊？ == 能否排除答案在左側的可能性？ |
| `[T T T F F F]` | 最後一個 T | **右界** | 向右   | `mid = ceil`  |                            |

mid = floor 時，我們的用物在於，取「左側最大的元素」來比較。如果他夠大了，那我們就將範圍留在左邊。不夠大，我們需要將範圍移動至右邊。

我們只需要夠大的值就好，不要最大，但也不能小於限制。
